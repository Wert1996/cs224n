...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         3. Right Arc
        if transition == "S":
            buffered_element = self.buffer.pop(0)
            self.stack.append(buffered_element)
        elif transition == "LA":
            first_element = self.stack.pop()
            second_element = self.stack.pop()
            self.dependencies.append((first_element, second_element))
            self.stack.append(first_element)
        elif transition == "RA":
            first_element = self.stack.pop()
            second_element = self.stack.pop()
            self.dependencies.append((second_element, first_element))
            self.stack.append(second_element)
        else:
